...
export const updateAvailableStandards = (
  ...
  if (framework && framework.standards) {
    ...
    filteredStandards.forEach((std) => console.log(`- ${std.id}: ${std.name}`));
...
export const processFrameworksResponse = (
  ...
  typedResponse.frameworks.forEach((fw: Framework) => {
    console.log(
      `Framework: ${fw.id} - ${fw.name} (${fw.standards?.length || 0} standards)`,
    );
    if (fw.standards && fw.standards.length > 0) {
      console.log(
        `Available standards: ${fw.standards
 ...
 */
export const logFrameworkSelection = (
  documentId: string,
  selectedFramework: string,
  selectedStandards: string[],
): void => {
  console.log(
    `Selecting framework ${selectedFramework} and standards: ${selectedStandards.join(", ")} for document ${documentId}`,
  );
};
