...
export async function exportComplianceResults(
  ...
  try {
    ...
    if (format === "excel") {
      ...
      await exportToPDF(exportData, results.metadata.company_name);
    }
...
async function exportToPDF(data: unknown, companyName: string): Promise<void> {
...
function generateCSVContent(data: unknown): string {
  const headers = [
    "Section",
    "Standard",
    "Question",
  ...
  // Add metadata row
  csvContent += `"METADATA","${data.metadata.company_name}","Framework: ${data.metadata.framework}","Standards: ${data.metadata.standards.join(", ")}","Compliance: ${data.metadata.compliance_percentage}%","Total Items: ${data.metadata.total_items}","Compliant: ${data.metadata.compliant_items}","Completed: ${data.metadata.completed_at || "N/A"}",""\n`;
  csvContent += "\n"; // Empty row

  // Add section data
  data.sections.forEach((section: unknown) => {
    section.items.forEach((item: unknown) => {
      const row = [
        `"${section.title}"`,
        `"${section.standard}"`,
