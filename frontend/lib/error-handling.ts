...
export class ApiErrorHandler {
  ...
  static parseError(error: unknown): ApiError {
    ...
    // Timeout errors
    if (error instanceof Error && (error.name === "AbortError" || (error as Error & { code?: string }).code === "TIMEOUT")) {
      return {
    ...
    if (error && typeof error === "object" && "response" in error) {
      const httpError = error as Error & { status?: number; response?: { status?: number; data?: unknown } };
      const status = httpError.response?.status;
      const data = httpError.response?.data as { detail?: string; details?: string } | undefined;
      ...
      switch (status) {
        ...
        default:
          return {
            message:
              data?.detail || `An unexpected error occurred (${status || 'unknown'}).`,
