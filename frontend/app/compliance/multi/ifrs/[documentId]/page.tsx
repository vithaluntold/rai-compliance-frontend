...
function ComplianceItem({
  ...
  item: ChecklistItem;
  onStatusChange: (itemId: string, status: ComplianceStatus, comments?: string) => void;
}) {
    ...
    const fetchChecklist = async () => {
      try {
        ...
        if (result.status === "COMPLETED" || result.status === "FAILED") {
          setData(result as ComplianceData); // Type casting to resolve compatibility issue
          if (result.sections) {
            const fetchedStandards = result.sections.map((section: Section) => ({
              id: section.section
                .toLowerCase()
                .replace(/ /g, "-")
                .replace(/[^a-z0-9-]/g, ""),
