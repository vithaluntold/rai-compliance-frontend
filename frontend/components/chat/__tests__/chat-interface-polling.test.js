 ...
 */
function mockHandleFileUpload(fileName, uploadResponse) {
  console.log(`\n🧪 TESTING: handleFileUpload(${fileName}, ${uploadResponse ? 'response provided' : 'no response'})`);
 ...
 */
function mockPollForMetadata(documentId) {
...
`);
  ...
  mockTestScenarios.forEach((scenario, index) => {
    ...
    const result = mockHandleFileUpload(testFileName, scenario.uploadResponse);
