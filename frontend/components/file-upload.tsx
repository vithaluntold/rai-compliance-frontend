...
export default function FileUpload({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env['NEXT_PUBLIC_API_URL'] || 'https://rai-compliance-backend.onrender.com'}/api/v1/upload`, {
        method: "POST",
        body: formData,
      });
...
setError(err instanceof Error ? err.message : "Upload failed");
      ...
      <div className="mt-6">
        <div
          ...
          className={`border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-8 text-center transition-colors ${
            isDragActive ? "border-blue-500 bg-blue-50 dark:bg-gray-800" : ""
          } ${isUploading ? "pointer-events-none opacity-50" : "cursor-pointer hover:border-gray-400 dark:hover:border-gray-500"}`}
        >
          <input {...getInputProps()} />
          <Upload className="mx-auto h-12 w-12 text-gray-400 dark:text-gray-500 mb-4" />
          
          {isDragActive ? (
            <p className="text-blue-600 dark:text-white">Drop the file here...</p>
          ) : (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow border dark:border-white">
        {currentStep === "upload" && (
          <div className="p-6">
            <motion.div
