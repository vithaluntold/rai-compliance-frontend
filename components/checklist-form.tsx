...
import type { AnalysisResults } from "@/lib/api-client";
...
export default function ChecklistForm({ fileId }: ChecklistFormProps) {
  const [checklist, setChecklist] = useState<AnalysisResults | null>(null);
  ...
  const isComplete = checklist?.sections?.every((section) =>
      section.items?.every((item) => item.status && item.status !== "N/A"),
    ) || false;
  ...
  useEffect(() => {
    const fetchChecklist = async () => {
      ...
      try {
        ...
        setChecklist(data as AnalysisResults);
      } catch {
        toast({
          title: "Error",
                      ...
                      <ChecklistItem
                        ...
                        onUpdate={(id: string, updates: Record<string, unknown>) => {
                          ...
                          if (checklist) {
                            ...
                            const sectionIndex = updatedChecklist.sections?.findIndex((s) =>
                            ...
                            if (
                              ...
                              if (itemIndex !== undefined && itemIndex >= 0) {
                                updatedChecklist.sections![sectionIndex].items![
                                  itemIndex
                                ] = {
                                  ...updatedChecklist.sections![sectionIndex]
                                    .items![itemIndex],
                                  ...updates,
                                };
