  ...
  useEffect(() => {
    if (keywords.length > animatedKeywords.length) {
      const timer = setTimeout(() => {
        setAnimatedKeywords((prev) => [...prev, keywords[prev.length]]);
      ...
      {isExtracting && (
        <div className="mt-3 space-y-1">
          ...
          <div className="w-full bg-gray-200 rounded-full h-1.5">
            <div
              className="bg-gradient-to-r from-[#0087d9] to-cyan-500 h-1.5 rounded-full transition-all duration-1000 animate-pulse"
              style={{
                width: '${Math.min(animatedKeywords.length * 20, 100)}%',
              }}
            />
          </div>
        </div>
