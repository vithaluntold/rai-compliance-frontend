...
export default function DocumentDetailsPage() {
    ...
    const checkStatus = async () => {
      try {
        ...
        if (typeof window !== "undefined") {
          console.warn("DEBUG: Status =", status.status, "MetadataExtraction =", status.metadata_extraction);
          // Log the complete response structure
          console.warn("Complete API Response:", JSON.stringify(status, null, 2));
    ...
    try {      
      ...
      if (response.metadata) {        
        ...
        const extractValue = (field: unknown) => {
          ...
          if (field && typeof field === 'object' && (field as any).value) return (field as any).value;
  ...
  const handleFrameworkSelectionError = (error: string) => {
    toast({
      title: "Error",
      description: error,
      variant: "destructive",
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center">
                <img src="/logo.png" alt="RAi" className="h-6 w-6" />
              </div>
              <div>
